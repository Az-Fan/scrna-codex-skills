...
Set and record the top-level integer `random_seed` before any enrichment task so permutation-based GSEA results are reproducible. The enrichment-only workflow defaults to seed 1 for backward compatibility, but formal project configs should state it explicitly.
...
Write full database-specific tables plus a combined table. Keep figures readable by producing a fixed-size GSEA overview with at most three terms per database and NES direction, a separate ORA overview, and bounded-height database-specific detail figures. Figures default to 300-dpi white-background PNG; create PDF only when `enrichment.plot_format` is explicitly `pdf` or `both`. Clean database prefixes and underscore-delimited pathway names before wrapping them; convert source labels that are entirely uppercase to sentence case while restoring common biological acronyms, and use narrower wrapping plus smaller pathway-label text in multi-panel overviews. GSEA uses a zero-centred diverging lollipop form: NES is position, direction is an explicit two-colour encoding, gene-set size is point size, filled versus open points distinguish FDR <= 0.05 from exploratory GSEA terms, and every displayed point carries a compact numeric q-value label so FDR evidence is directly visible. Database-specific GSEA details keep both NES directions on one shared zero-centred axis so labels and data retain usable width. ORA combines both input directions in one zero-centred panel per database: signed rich factor conveys direction and magnitude, line and border colour reinforce direction, point fill conveys FDR, and point size conveys gene count. The plotted-term summary records the exact plotted x value, size value, FDR label, and their source metrics for audit. Detail heights adapt to the number of displayed terms so sparse results do not create mostly empty pages. Use configurable population, comparison, positive-direction, and negative-direction display labels so plot semantics do not depend on internal table identifiers.
