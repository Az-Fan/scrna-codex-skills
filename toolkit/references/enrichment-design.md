...
Write full database-specific tables plus a combined table. Keep figures readable by producing a fixed-size GSEA overview with at most three terms per database and NES direction, a separate ORA overview, and bounded-height database-specific detail PDFs. Clean database prefixes and underscore-delimited pathway names before wrapping them; use narrower wrapping in multi-column overviews so long pathway names stay within their facets. GSEA uses a zero-centred diverging lollipop form: NES is position, direction is an explicit two-colour encoding, gene-set size is point size, and filled versus open points distinguish FDR <= 0.05 from exploratory GSEA terms. ORA uses rich factor, gene count, FDR, and faceted input direction. Use configurable population, comparison, positive-direction, and negative-direction display labels so plot semantics do not depend on internal table identifiers.
